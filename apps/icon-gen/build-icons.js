 ...
 * TODO: typescript future
 */

const cwd = process.cwd()
const inputDir = path.join(cwd, 'svg-icons')
const inputDirRelative = path.relative(cwd, inputDir)
const outputSpriteSheetDir = path.join(
  cwd,
  '..',
  'web',
  'public',
...
const outputSpriteTypeDir = path.join(
  cwd,
  '../../',
  'packages',
  'ui',
...
const outputDirRelative = path.relative(cwd, outputSpriteSheetDir)
const files = glob
  .sync('**/*.svg', {
    cwd: inputDir,
  })
...
await writeIfChanged(
  path.join(outputSpriteSheetDir, 'sprite.svg'),
  spritesheetContent,
)

const typesContent = await generateTypes({
  names: files.map(file => file.replace(/\.svg$/, '')), // files.map(file => JSON.stringify(file => file.replace(/\.svg$/, ''))),
})
await writeIfChanged(path.join(outputSpriteTypeDir, 'names.ts'), typesContent)
