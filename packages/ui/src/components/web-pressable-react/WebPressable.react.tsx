...
import { isBrowser } from '@ui/utils/user-agent'
...
function shouldHandleClickEvent(event: any, b: any) {
...
const WebPressableReact = (props: any) => {
  ...
  var Z = accessibilityState == null ? undefined : accessibilityState.hidden

  const $ = Y === 'a' && _disabled !== true

  const _focusVisibleChangeState = {
    disabled:
      _disabled === true ||
  ...
  const _focusChangeState =
    typeof allowClickEventPropagation === 'function'
      ? allowClickEventPropagation(_focusVisibleChangeState)
      : allowClickEventPropagation

  const _hoverChangeState =
    typeof className_DEPRECATED === 'function'
      ? className_DEPRECATED(_focusVisibleChangeState)
      : className_DEPRECATED

  const _pressChangeState =
    typeof style === 'function' ? style(_focusVisibleChangeState) : style

  const _testOnly_state =
    typeof className === 'function'
      ? className(_focusVisibleChangeState)
      : className
  ...
  allowClickEventPropagation = useCallback(
    function (a) {
      onPress && onPress(a),
        (onPress || link != null) &&
          allowClickEventPropagation !== true &&
          a.stopPropagation(),
        shouldHandleClickEvent(a, preventDefault) &&
          a.nativeEvent.preventDefault()
    },
  ...
  // c('gkx')('5403')
  // ? Z !== true && tabbable !== false && (xstyle = 0):
  _disabled !== true && Z !== true && tabbable !== false && (xstyle = 0)
  ...
  return jsx(
    ...
    Object.assign({}, rest, {
      ...
      className: mergeClasses(
        classes.root,
        focusVisibleChangeState.disabled && classes.disabled,
        (!focusVisibleChangeState.focusVisible || suppressFocusRing === true) &&
          classes.focusNotVisible,
        testOnly_state,
        WebPressableGroupContextValue && classes.rootInGroup,
        hoverChangeState,
